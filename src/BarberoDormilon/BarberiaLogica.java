...
class BarberoTienda {
        ...
        synchronized (listCustomer) {
            while (true) {
                while (listCustomer.size() == 0 || sillas == 0) {
                    textote.append("Barbero " + val + " está esperando al cliente\n");
                    try {
                        listCustomer.wait();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                ...
                textote.append("Barbero " + val + " encontró cliente en cola\n");
    ...
    public void add(Customer customer, TextArea textote) {
        textote.append("Cliente : " + customer.getName() + " entró a la tienda\n");
        synchronized (listCustomer) {
            while (sillas == 0) {
